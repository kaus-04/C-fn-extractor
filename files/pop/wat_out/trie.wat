(module $trie.wasm
  (type (;0;) (func (param i32 i32) (result i32)))
  (type (;1;) (func (param i32 i32 i32) (result i32)))
  (type (;2;) (func (param i32)))
  (type (;3;) (func))
  (type (;4;) (func (param i32) (result i32)))
  (type (;5;) (func (param i32 i32 i32 i32) (result i32)))
  (type (;6;) (func (param i32 i32 i32)))
  (type (;7;) (func (result i32)))
  (import "env" "calloc" (func $calloc (type 0)))
  (import "env" "printf" (func $printf (type 0)))
  (import "env" "fprintf" (func $fprintf (type 1)))
  (import "env" "exit" (func $exit (type 2)))
  (import "env" "fopen" (func $fopen (type 0)))
  (import "env" "fscanf" (func $fscanf (type 1)))
  (import "env" "strnlen" (func $strnlen (type 0)))
  (import "env" "scanf" (func $scanf (type 0)))
  (func $__wasm_call_ctors (type 3))
  (func $trie_new (type 4) (param i32) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 1
    i32.const 16
    local.set 2
    local.get 1
    local.get 2
    i32.sub
    local.set 3
    local.get 3
    global.set $__stack_pointer
    local.get 3
    local.get 0
    i32.store offset=8
    i32.const 1
    local.set 4
    i32.const 108
    local.set 5
    local.get 4
    local.get 5
    call $calloc
    local.set 6
    local.get 3
    i32.load offset=8
    local.set 7
    local.get 7
    local.get 6
    i32.store
    local.get 3
    i32.load offset=8
    local.set 8
    local.get 8
    i32.load
    local.set 9
    i32.const 0
    local.set 10
    local.get 10
    local.get 9
    i32.eq
    local.set 11
    i32.const 1
    local.set 12
    local.get 11
    local.get 12
    i32.and
    local.set 13
    block  ;; label = @1
      block  ;; label = @2
        local.get 13
        i32.eqz
        br_if 0 (;@2;)
        i32.const -1
        local.set 14
        local.get 3
        local.get 14
        i32.store offset=12
        br 1 (;@1;)
      end
      i32.const 0
      local.set 15
      local.get 3
      local.get 15
      i32.store offset=12
    end
    local.get 3
    i32.load offset=12
    local.set 16
    i32.const 16
    local.set 17
    local.get 3
    local.get 17
    i32.add
    local.set 18
    local.get 18
    global.set $__stack_pointer
    local.get 16
    return)
  (func $trie_insert (type 1) (param i32 i32 i32) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 3
    i32.const 32
    local.set 4
    local.get 3
    local.get 4
    i32.sub
    local.set 5
    local.get 5
    global.set $__stack_pointer
    local.get 5
    local.get 0
    i32.store offset=24
    local.get 5
    local.get 1
    i32.store offset=20
    local.get 5
    local.get 2
    i32.store offset=16
    i32.const 0
    local.set 6
    local.get 5
    local.get 6
    i32.store offset=12
    local.get 5
    i32.load offset=16
    local.set 7
    i32.const 0
    local.set 8
    local.get 8
    local.get 7
    i32.eq
    local.set 9
    i32.const 1
    local.set 10
    local.get 9
    local.get 10
    i32.and
    local.set 11
    block  ;; label = @1
      block  ;; label = @2
        local.get 11
        i32.eqz
        br_if 0 (;@2;)
        local.get 5
        i32.load offset=24
        local.set 12
        i32.const 1
        local.set 13
        local.get 12
        local.get 13
        i32.store8 offset=104
        i32.const 0
        local.set 14
        local.get 5
        local.get 14
        i32.store offset=28
        br 1 (;@1;)
      end
      local.get 5
      i32.load offset=20
      local.set 15
      local.get 15
      i32.load8_u
      local.set 16
      i32.const 24
      local.set 17
      local.get 16
      local.get 17
      i32.shl
      local.set 18
      local.get 18
      local.get 17
      i32.shr_s
      local.set 19
      i32.const 97
      local.set 20
      local.get 19
      local.get 20
      i32.sub
      local.set 21
      local.get 5
      local.get 21
      i32.store offset=8
      local.get 5
      i32.load offset=8
      local.set 22
      i32.const 26
      local.set 23
      local.get 23
      local.get 22
      i32.le_u
      local.set 24
      i32.const 1
      local.set 25
      local.get 24
      local.get 25
      i32.and
      local.set 26
      block  ;; label = @2
        local.get 26
        i32.eqz
        br_if 0 (;@2;)
        i32.const -1
        local.set 27
        local.get 5
        local.get 27
        i32.store offset=28
        br 1 (;@1;)
      end
      local.get 5
      i32.load offset=24
      local.set 28
      local.get 5
      i32.load offset=8
      local.set 29
      i32.const 2
      local.set 30
      local.get 29
      local.get 30
      i32.shl
      local.set 31
      local.get 28
      local.get 31
      i32.add
      local.set 32
      local.get 32
      i32.load
      local.set 33
      i32.const 0
      local.set 34
      local.get 34
      local.get 33
      i32.eq
      local.set 35
      i32.const 1
      local.set 36
      local.get 35
      local.get 36
      i32.and
      local.set 37
      block  ;; label = @2
        local.get 37
        i32.eqz
        br_if 0 (;@2;)
        local.get 5
        i32.load offset=24
        local.set 38
        local.get 5
        i32.load offset=8
        local.set 39
        i32.const 2
        local.set 40
        local.get 39
        local.get 40
        i32.shl
        local.set 41
        local.get 38
        local.get 41
        i32.add
        local.set 42
        local.get 42
        call $trie_new
        local.set 43
        local.get 5
        local.get 43
        i32.store offset=12
        local.get 5
        i32.load offset=12
        local.set 44
        i32.const -1
        local.set 45
        local.get 45
        local.get 44
        i32.eq
        local.set 46
        i32.const 1
        local.set 47
        local.get 46
        local.get 47
        i32.and
        local.set 48
        block  ;; label = @3
          local.get 48
          i32.eqz
          br_if 0 (;@3;)
          i32.const -1
          local.set 49
          local.get 5
          local.get 49
          i32.store offset=28
          br 2 (;@1;)
        end
      end
      local.get 5
      i32.load offset=24
      local.set 50
      local.get 5
      i32.load offset=8
      local.set 51
      i32.const 2
      local.set 52
      local.get 51
      local.get 52
      i32.shl
      local.set 53
      local.get 50
      local.get 53
      i32.add
      local.set 54
      local.get 54
      i32.load
      local.set 55
      local.get 5
      i32.load offset=20
      local.set 56
      i32.const 1
      local.set 57
      local.get 56
      local.get 57
      i32.add
      local.set 58
      local.get 5
      i32.load offset=16
      local.set 59
      i32.const 1
      local.set 60
      local.get 59
      local.get 60
      i32.sub
      local.set 61
      local.get 55
      local.get 58
      local.get 61
      call $trie_insert
      local.set 62
      local.get 5
      local.get 62
      i32.store offset=28
    end
    local.get 5
    i32.load offset=28
    local.set 63
    i32.const 32
    local.set 64
    local.get 5
    local.get 64
    i32.add
    local.set 65
    local.get 65
    global.set $__stack_pointer
    local.get 63
    return)
  (func $trie_search (type 5) (param i32 i32 i32 i32) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 4
    i32.const 32
    local.set 5
    local.get 4
    local.get 5
    i32.sub
    local.set 6
    local.get 6
    global.set $__stack_pointer
    local.get 6
    local.get 0
    i32.store offset=24
    local.get 6
    local.get 1
    i32.store offset=20
    local.get 6
    local.get 2
    i32.store offset=16
    local.get 6
    local.get 3
    i32.store offset=12
    local.get 6
    i32.load offset=16
    local.set 7
    i32.const 0
    local.set 8
    local.get 8
    local.get 7
    i32.eq
    local.set 9
    i32.const 1
    local.set 10
    local.get 9
    local.get 10
    i32.and
    local.set 11
    block  ;; label = @1
      block  ;; label = @2
        local.get 11
        i32.eqz
        br_if 0 (;@2;)
        local.get 6
        i32.load offset=24
        local.set 12
        local.get 6
        i32.load offset=12
        local.set 13
        local.get 13
        local.get 12
        i32.store
        i32.const 0
        local.set 14
        local.get 6
        local.get 14
        i32.store offset=28
        br 1 (;@1;)
      end
      local.get 6
      i32.load offset=20
      local.set 15
      local.get 15
      i32.load8_u
      local.set 16
      i32.const 24
      local.set 17
      local.get 16
      local.get 17
      i32.shl
      local.set 18
      local.get 18
      local.get 17
      i32.shr_s
      local.set 19
      i32.const 97
      local.set 20
      local.get 19
      local.get 20
      i32.sub
      local.set 21
      local.get 6
      local.get 21
      i32.store offset=8
      local.get 6
      i32.load offset=8
      local.set 22
      i32.const 26
      local.set 23
      local.get 23
      local.get 22
      i32.le_u
      local.set 24
      i32.const 1
      local.set 25
      local.get 24
      local.get 25
      i32.and
      local.set 26
      block  ;; label = @2
        local.get 26
        i32.eqz
        br_if 0 (;@2;)
        i32.const -1
        local.set 27
        local.get 6
        local.get 27
        i32.store offset=28
        br 1 (;@1;)
      end
      local.get 6
      i32.load offset=24
      local.set 28
      local.get 6
      i32.load offset=8
      local.set 29
      i32.const 2
      local.set 30
      local.get 29
      local.get 30
      i32.shl
      local.set 31
      local.get 28
      local.get 31
      i32.add
      local.set 32
      local.get 32
      i32.load
      local.set 33
      i32.const 0
      local.set 34
      local.get 34
      local.get 33
      i32.eq
      local.set 35
      i32.const 1
      local.set 36
      local.get 35
      local.get 36
      i32.and
      local.set 37
      block  ;; label = @2
        local.get 37
        i32.eqz
        br_if 0 (;@2;)
        i32.const -1
        local.set 38
        local.get 6
        local.get 38
        i32.store offset=28
        br 1 (;@1;)
      end
      local.get 6
      i32.load offset=24
      local.set 39
      local.get 6
      i32.load offset=8
      local.set 40
      i32.const 2
      local.set 41
      local.get 40
      local.get 41
      i32.shl
      local.set 42
      local.get 39
      local.get 42
      i32.add
      local.set 43
      local.get 43
      i32.load
      local.set 44
      local.get 6
      i32.load offset=20
      local.set 45
      i32.const 1
      local.set 46
      local.get 45
      local.get 46
      i32.add
      local.set 47
      local.get 6
      i32.load offset=16
      local.set 48
      i32.const 1
      local.set 49
      local.get 48
      local.get 49
      i32.sub
      local.set 50
      local.get 6
      i32.load offset=12
      local.set 51
      local.get 44
      local.get 47
      local.get 50
      local.get 51
      call $trie_search
      local.set 52
      local.get 6
      local.get 52
      i32.store offset=28
    end
    local.get 6
    i32.load offset=28
    local.set 53
    i32.const 32
    local.set 54
    local.get 6
    local.get 54
    i32.add
    local.set 55
    local.get 55
    global.set $__stack_pointer
    local.get 53
    return)
  (func $trie_print (type 6) (param i32 i32 i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 3
    i32.const 32
    local.set 4
    local.get 3
    local.get 4
    i32.sub
    local.set 5
    local.get 5
    global.set $__stack_pointer
    local.get 5
    local.get 0
    i32.store offset=28
    local.get 5
    local.get 1
    i32.store offset=24
    local.get 5
    local.get 2
    i32.store offset=20
    local.get 5
    i32.load offset=28
    local.set 6
    local.get 6
    i32.load8_u offset=104
    local.set 7
    i32.const 1
    local.set 8
    local.get 7
    local.get 8
    i32.and
    local.set 9
    i32.const 1
    local.set 10
    local.get 10
    local.get 9
    i32.eq
    local.set 11
    i32.const 1
    local.set 12
    local.get 11
    local.get 12
    i32.and
    local.set 13
    block  ;; label = @1
      local.get 13
      i32.eqz
      br_if 0 (;@1;)
      local.get 5
      i32.load offset=20
      local.set 14
      local.get 5
      i32.load offset=24
      local.set 15
      local.get 5
      local.get 15
      i32.store offset=4
      local.get 5
      local.get 14
      i32.store
      i32.const 1118
      local.set 16
      local.get 16
      local.get 5
      call $printf
      drop
    end
    i32.const 0
    local.set 17
    local.get 5
    local.get 17
    i32.store offset=16
    block  ;; label = @1
      loop  ;; label = @2
        local.get 5
        i32.load offset=16
        local.set 18
        i32.const 26
        local.set 19
        local.get 18
        local.get 19
        i32.lt_s
        local.set 20
        i32.const 1
        local.set 21
        local.get 20
        local.get 21
        i32.and
        local.set 22
        local.get 22
        i32.eqz
        br_if 1 (;@1;)
        local.get 5
        i32.load offset=28
        local.set 23
        local.get 5
        i32.load offset=16
        local.set 24
        i32.const 2
        local.set 25
        local.get 24
        local.get 25
        i32.shl
        local.set 26
        local.get 23
        local.get 26
        i32.add
        local.set 27
        local.get 27
        i32.load
        local.set 28
        i32.const 0
        local.set 29
        local.get 29
        local.get 28
        i32.eq
        local.set 30
        i32.const 1
        local.set 31
        local.get 30
        local.get 31
        i32.and
        local.set 32
        block  ;; label = @3
          block  ;; label = @4
            local.get 32
            i32.eqz
            br_if 0 (;@4;)
            br 1 (;@3;)
          end
          local.get 5
          i32.load offset=16
          local.set 33
          i32.const 97
          local.set 34
          local.get 33
          local.get 34
          i32.add
          local.set 35
          local.get 5
          i32.load offset=24
          local.set 36
          local.get 5
          i32.load offset=20
          local.set 37
          local.get 36
          local.get 37
          i32.add
          local.set 38
          local.get 38
          local.get 35
          i32.store8
          local.get 5
          i32.load offset=28
          local.set 39
          local.get 5
          i32.load offset=16
          local.set 40
          i32.const 2
          local.set 41
          local.get 40
          local.get 41
          i32.shl
          local.set 42
          local.get 39
          local.get 42
          i32.add
          local.set 43
          local.get 43
          i32.load
          local.set 44
          local.get 5
          i32.load offset=24
          local.set 45
          local.get 5
          i32.load offset=20
          local.set 46
          i32.const 1
          local.set 47
          local.get 46
          local.get 47
          i32.add
          local.set 48
          local.get 44
          local.get 45
          local.get 48
          call $trie_print
        end
        local.get 5
        i32.load offset=16
        local.set 49
        i32.const 1
        local.set 50
        local.get 49
        local.get 50
        i32.add
        local.set 51
        local.get 5
        local.get 51
        i32.store offset=16
        br 0 (;@2;)
      end
    end
    i32.const 32
    local.set 52
    local.get 5
    local.get 52
    i32.add
    local.set 53
    local.get 53
    global.set $__stack_pointer
    return)
  (func $__original_main (type 7) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 160
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 2
    local.get 3
    i32.store offset=156
    i32.const 0
    local.set 4
    local.get 2
    local.get 4
    i32.store offset=152
    i32.const 0
    local.set 5
    local.get 2
    local.get 5
    i32.store offset=148
    i32.const 0
    local.set 6
    local.get 2
    local.get 6
    i32.store offset=144
    i32.const 100
    local.set 7
    i32.const 0
    local.set 8
    local.get 7
    i32.eqz
    local.set 9
    block  ;; label = @1
      local.get 9
      br_if 0 (;@1;)
      i32.const 32
      local.set 10
      local.get 2
      local.get 10
      i32.add
      local.set 11
      local.get 11
      local.get 8
      local.get 7
      memory.fill
    end
    i32.const 148
    local.set 12
    local.get 2
    local.get 12
    i32.add
    local.set 13
    local.get 13
    local.set 14
    local.get 14
    call $trie_new
    local.set 15
    local.get 2
    local.get 15
    i32.store offset=152
    local.get 2
    i32.load offset=152
    local.set 16
    i32.const -1
    local.set 17
    local.get 17
    local.get 16
    i32.eq
    local.set 18
    i32.const 1
    local.set 19
    local.get 18
    local.get 19
    i32.and
    local.set 20
    block  ;; label = @1
      local.get 20
      i32.eqz
      br_if 0 (;@1;)
      i32.const 0
      local.set 21
      local.get 21
      i32.load
      local.set 22
      i32.const 1157
      local.set 23
      i32.const 0
      local.set 24
      local.get 22
      local.get 23
      local.get 24
      call $fprintf
      drop
      i32.const 1
      local.set 25
      local.get 25
      call $exit
      unreachable
    end
    i32.const 1024
    local.set 26
    i32.const 1045
    local.set 27
    local.get 26
    local.get 27
    call $fopen
    local.set 28
    local.get 2
    local.get 28
    i32.store offset=28
    local.get 2
    i32.load offset=28
    local.set 29
    i32.const 0
    local.set 30
    local.get 30
    local.get 29
    i32.eq
    local.set 31
    i32.const 1
    local.set 32
    local.get 31
    local.get 32
    i32.and
    local.set 33
    block  ;; label = @1
      local.get 33
      i32.eqz
      br_if 0 (;@1;)
      i32.const 0
      local.set 34
      local.get 34
      i32.load
      local.set 35
      i32.const 1047
      local.set 36
      i32.const 0
      local.set 37
      local.get 35
      local.get 36
      local.get 37
      call $fprintf
      drop
      i32.const 1
      local.set 38
      local.get 38
      call $exit
      unreachable
    end
    block  ;; label = @1
      loop  ;; label = @2
        local.get 2
        i32.load offset=28
        local.set 39
        i32.const 32
        local.set 40
        local.get 2
        local.get 40
        i32.add
        local.set 41
        local.get 41
        local.set 42
        local.get 2
        local.get 42
        i32.store offset=16
        i32.const 1111
        local.set 43
        i32.const 16
        local.set 44
        local.get 2
        local.get 44
        i32.add
        local.set 45
        local.get 39
        local.get 43
        local.get 45
        call $fscanf
        local.set 46
        i32.const 1
        local.set 47
        local.get 47
        local.get 46
        i32.eq
        local.set 48
        i32.const 1
        local.set 49
        local.get 48
        local.get 49
        i32.and
        local.set 50
        local.get 50
        i32.eqz
        br_if 1 (;@1;)
        local.get 2
        i32.load offset=148
        local.set 51
        i32.const 32
        local.set 52
        local.get 2
        local.get 52
        i32.add
        local.set 53
        local.get 53
        local.set 54
        i32.const 32
        local.set 55
        local.get 2
        local.get 55
        i32.add
        local.set 56
        local.get 56
        local.set 57
        i32.const 100
        local.set 58
        local.get 57
        local.get 58
        call $strnlen
        local.set 59
        local.get 51
        local.get 54
        local.get 59
        call $trie_insert
        local.set 60
        local.get 2
        local.get 60
        i32.store offset=152
        local.get 2
        i32.load offset=152
        local.set 61
        i32.const -1
        local.set 62
        local.get 62
        local.get 61
        i32.eq
        local.set 63
        i32.const 1
        local.set 64
        local.get 63
        local.get 64
        i32.and
        local.set 65
        block  ;; label = @3
          local.get 65
          i32.eqz
          br_if 0 (;@3;)
          i32.const 0
          local.set 66
          local.get 66
          i32.load
          local.set 67
          i32.const 1124
          local.set 68
          i32.const 0
          local.set 69
          local.get 67
          local.get 68
          local.get 69
          call $fprintf
          drop
          i32.const 1
          local.set 70
          local.get 70
          call $exit
          unreachable
        end
        br 0 (;@2;)
      end
    end
    block  ;; label = @1
      loop  ;; label = @2
        i32.const 1083
        local.set 71
        i32.const 0
        local.set 72
        local.get 71
        local.get 72
        call $printf
        drop
        i32.const 32
        local.set 73
        local.get 2
        local.get 73
        i32.add
        local.set 74
        local.get 74
        local.set 75
        local.get 2
        local.get 75
        i32.store
        i32.const 1039
        local.set 76
        local.get 76
        local.get 2
        call $scanf
        local.set 77
        i32.const 1
        local.set 78
        local.get 78
        local.get 77
        i32.ne
        local.set 79
        i32.const 1
        local.set 80
        local.get 79
        local.get 80
        i32.and
        local.set 81
        block  ;; label = @3
          local.get 81
          i32.eqz
          br_if 0 (;@3;)
          br 2 (;@1;)
        end
        i32.const 1180
        local.set 82
        i32.const 0
        local.set 83
        local.get 82
        local.get 83
        call $printf
        drop
        i32.const 1241
        local.set 84
        i32.const 0
        local.set 85
        local.get 84
        local.get 85
        call $printf
        drop
        local.get 2
        i32.load offset=148
        local.set 86
        i32.const 32
        local.set 87
        local.get 2
        local.get 87
        i32.add
        local.set 88
        local.get 88
        local.set 89
        i32.const 32
        local.set 90
        local.get 2
        local.get 90
        i32.add
        local.set 91
        local.get 91
        local.set 92
        i32.const 100
        local.set 93
        local.get 92
        local.get 93
        call $strnlen
        local.set 94
        i32.const 144
        local.set 95
        local.get 2
        local.get 95
        i32.add
        local.set 96
        local.get 96
        local.set 97
        local.get 86
        local.get 89
        local.get 94
        local.get 97
        call $trie_search
        local.set 98
        local.get 2
        local.get 98
        i32.store offset=152
        local.get 2
        i32.load offset=152
        local.set 99
        i32.const -1
        local.set 100
        local.get 100
        local.get 99
        i32.eq
        local.set 101
        i32.const 1
        local.set 102
        local.get 101
        local.get 102
        i32.and
        local.set 103
        block  ;; label = @3
          local.get 103
          i32.eqz
          br_if 0 (;@3;)
          i32.const 1099
          local.set 104
          i32.const 0
          local.set 105
          local.get 104
          local.get 105
          call $printf
          drop
          br 1 (;@2;)
        end
        local.get 2
        i32.load offset=144
        local.set 106
        i32.const 32
        local.set 107
        local.get 2
        local.get 107
        i32.add
        local.set 108
        local.get 108
        local.set 109
        i32.const 32
        local.set 110
        local.get 2
        local.get 110
        i32.add
        local.set 111
        local.get 111
        local.set 112
        i32.const 100
        local.set 113
        local.get 112
        local.get 113
        call $strnlen
        local.set 114
        local.get 106
        local.get 109
        local.get 114
        call $trie_print
        i32.const 1180
        local.set 115
        i32.const 0
        local.set 116
        local.get 115
        local.get 116
        call $printf
        drop
        br 0 (;@2;)
      end
    end
    local.get 2
    i32.load offset=156
    local.set 117
    i32.const 160
    local.set 118
    local.get 2
    local.get 118
    i32.add
    local.set 119
    local.get 119
    global.set $__stack_pointer
    local.get 117
    return)
  (func $main (type 0) (param i32 i32) (result i32)
    (local i32)
    call $__original_main
    local.set 2
    local.get 2
    return)
  (table (;0;) 1 1 funcref)
  (memory (;0;) 2)
  (global $__stack_pointer (mut i32) (i32.const 66848))
  (global (;1;) i32 (i32.const 1024))
  (global (;2;) i32 (i32.const 1301))
  (global (;3;) i32 (i32.const 1312))
  (global (;4;) i32 (i32.const 66848))
  (global (;5;) i32 (i32.const 1024))
  (global (;6;) i32 (i32.const 66848))
  (global (;7;) i32 (i32.const 131072))
  (global (;8;) i32 (i32.const 0))
  (global (;9;) i32 (i32.const 1))
  (export "memory" (memory 0))
  (export "__wasm_call_ctors" (func $__wasm_call_ctors))
  (export "trie_new" (func $trie_new))
  (export "trie_insert" (func $trie_insert))
  (export "trie_search" (func $trie_search))
  (export "trie_print" (func $trie_print))
  (export "__original_main" (func $__original_main))
  (export "main" (func $main))
  (export "__main_void" (func $__original_main))
  (export "__indirect_function_table" (table 0))
  (export "__dso_handle" (global 1))
  (export "__data_end" (global 2))
  (export "__stack_low" (global 3))
  (export "__stack_high" (global 4))
  (export "__global_base" (global 5))
  (export "__heap_base" (global 6))
  (export "__heap_end" (global 7))
  (export "__memory_base" (global 8))
  (export "__table_base" (global 9))
  (data $.rodata (i32.const 1024) "dictionary.txt\00%100s\00r\00Error while opening dictionary file\00Enter keyword: \00No results\0a\00%100s\0a\00%.*s\0a\00Could not insert word into trie\0a\00Could not create trie\0a\00\0a==========================================================\0a\00\0a********************* Possible Words ********************\0a\00"))
