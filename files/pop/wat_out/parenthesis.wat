(module $parenthesis.wasm
  (type (;0;) (func (param i32) (result i32)))
  (type (;1;) (func (param i32)))
  (type (;2;) (func (param i32 i32) (result i32)))
  (type (;3;) (func))
  (type (;4;) (func (result i32)))
  (import "env" "malloc" (func $malloc (type 0)))
  (import "env" "free" (func $free (type 1)))
  (import "env" "printf" (func $printf (type 2)))
  (import "env" "scanf" (func $scanf (type 2)))
  (func $__wasm_call_ctors (type 3))
  (func $push (type 1) (param i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 1
    i32.const 16
    local.set 2
    local.get 1
    local.get 2
    i32.sub
    local.set 3
    local.get 3
    global.set $__stack_pointer
    local.get 3
    local.get 0
    i32.store8 offset=15
    i32.const 0
    local.set 4
    local.get 4
    i32.load offset=1152
    local.set 5
    local.get 3
    local.get 5
    i32.store offset=8
    i32.const 8
    local.set 6
    local.get 6
    call $malloc
    local.set 7
    local.get 3
    local.get 7
    i32.store offset=4
    local.get 3
    i32.load8_u offset=15
    local.set 8
    local.get 3
    i32.load offset=4
    local.set 9
    local.get 9
    local.get 8
    i32.store8
    i32.const 0
    local.set 10
    local.get 10
    i32.load offset=1152
    local.set 11
    i32.const 0
    local.set 12
    local.get 11
    local.get 12
    i32.eq
    local.set 13
    i32.const 1
    local.set 14
    local.get 13
    local.get 14
    i32.and
    local.set 15
    block  ;; label = @1
      block  ;; label = @2
        local.get 15
        i32.eqz
        br_if 0 (;@2;)
        local.get 3
        i32.load offset=4
        local.set 16
        i32.const 0
        local.set 17
        local.get 17
        local.get 16
        i32.store offset=1152
        i32.const 0
        local.set 18
        local.get 18
        i32.load offset=1152
        local.set 19
        local.get 3
        local.get 19
        i32.store offset=8
        local.get 3
        i32.load offset=8
        local.set 20
        i32.const 0
        local.set 21
        local.get 20
        local.get 21
        i32.store offset=4
        i32.const 0
        local.set 22
        local.get 22
        i32.load offset=1148
        local.set 23
        i32.const 1
        local.set 24
        local.get 23
        local.get 24
        i32.add
        local.set 25
        i32.const 0
        local.set 26
        local.get 26
        local.get 25
        i32.store offset=1148
        br 1 (;@1;)
      end
      local.get 3
      i32.load offset=8
      local.set 27
      local.get 3
      i32.load offset=4
      local.set 28
      local.get 28
      local.get 27
      i32.store offset=4
      local.get 3
      i32.load offset=4
      local.set 29
      local.get 3
      local.get 29
      i32.store offset=8
      local.get 3
      i32.load offset=8
      local.set 30
      i32.const 0
      local.set 31
      local.get 31
      local.get 30
      i32.store offset=1152
      i32.const 0
      local.set 32
      local.get 32
      i32.load offset=1148
      local.set 33
      i32.const 1
      local.set 34
      local.get 33
      local.get 34
      i32.add
      local.set 35
      i32.const 0
      local.set 36
      local.get 36
      local.get 35
      i32.store offset=1148
    end
    i32.const 16
    local.set 37
    local.get 3
    local.get 37
    i32.add
    local.set 38
    local.get 38
    global.set $__stack_pointer
    return)
  (func $pop (type 4) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 16
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 3
    i32.load offset=1152
    local.set 4
    local.get 2
    local.get 4
    i32.store offset=8
    local.get 2
    i32.load offset=8
    local.set 5
    local.get 5
    i32.load8_u
    local.set 6
    local.get 2
    local.get 6
    i32.store8 offset=15
    local.get 2
    i32.load offset=8
    local.set 7
    local.get 7
    i32.load offset=4
    local.set 8
    i32.const 0
    local.set 9
    local.get 9
    local.get 8
    i32.store offset=1152
    local.get 2
    i32.load offset=8
    local.set 10
    local.get 10
    call $free
    i32.const 0
    local.set 11
    local.get 11
    i32.load offset=1148
    local.set 12
    i32.const -1
    local.set 13
    local.get 12
    local.get 13
    i32.add
    local.set 14
    i32.const 0
    local.set 15
    local.get 15
    local.get 14
    i32.store offset=1148
    local.get 2
    i32.load8_u offset=15
    local.set 16
    i32.const 24
    local.set 17
    local.get 16
    local.get 17
    i32.shl
    local.set 18
    local.get 18
    local.get 17
    i32.shr_s
    local.set 19
    i32.const 16
    local.set 20
    local.get 2
    local.get 20
    i32.add
    local.set 21
    local.get 21
    global.set $__stack_pointer
    local.get 19
    return)
  (func $isBalanced (type 0) (param i32) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 1
    i32.const 16
    local.set 2
    local.get 1
    local.get 2
    i32.sub
    local.set 3
    local.get 3
    global.set $__stack_pointer
    local.get 3
    local.get 0
    i32.store offset=8
    i32.const 0
    local.set 4
    local.get 3
    local.get 4
    i32.store offset=4
    block  ;; label = @1
      block  ;; label = @2
        loop  ;; label = @3
          local.get 3
          i32.load offset=8
          local.set 5
          local.get 3
          i32.load offset=4
          local.set 6
          local.get 5
          local.get 6
          i32.add
          local.set 7
          local.get 7
          i32.load8_u
          local.set 8
          i32.const 24
          local.set 9
          local.get 8
          local.get 9
          i32.shl
          local.set 10
          local.get 10
          local.get 9
          i32.shr_s
          local.set 11
          local.get 11
          i32.eqz
          br_if 1 (;@2;)
          local.get 3
          i32.load offset=8
          local.set 12
          local.get 3
          i32.load offset=4
          local.set 13
          local.get 12
          local.get 13
          i32.add
          local.set 14
          local.get 14
          i32.load8_u
          local.set 15
          i32.const 24
          local.set 16
          local.get 15
          local.get 16
          i32.shl
          local.set 17
          local.get 17
          local.get 16
          i32.shr_s
          local.set 18
          i32.const 123
          local.set 19
          local.get 18
          local.get 19
          i32.eq
          local.set 20
          i32.const 1
          local.set 21
          local.get 20
          local.get 21
          i32.and
          local.set 22
          block  ;; label = @4
            block  ;; label = @5
              block  ;; label = @6
                local.get 22
                br_if 0 (;@6;)
                local.get 3
                i32.load offset=8
                local.set 23
                local.get 3
                i32.load offset=4
                local.set 24
                local.get 23
                local.get 24
                i32.add
                local.set 25
                local.get 25
                i32.load8_u
                local.set 26
                i32.const 24
                local.set 27
                local.get 26
                local.get 27
                i32.shl
                local.set 28
                local.get 28
                local.get 27
                i32.shr_s
                local.set 29
                i32.const 40
                local.set 30
                local.get 29
                local.get 30
                i32.eq
                local.set 31
                i32.const 1
                local.set 32
                local.get 31
                local.get 32
                i32.and
                local.set 33
                local.get 33
                br_if 0 (;@6;)
                local.get 3
                i32.load offset=8
                local.set 34
                local.get 3
                i32.load offset=4
                local.set 35
                local.get 34
                local.get 35
                i32.add
                local.set 36
                local.get 36
                i32.load8_u
                local.set 37
                i32.const 24
                local.set 38
                local.get 37
                local.get 38
                i32.shl
                local.set 39
                local.get 39
                local.get 38
                i32.shr_s
                local.set 40
                i32.const 91
                local.set 41
                local.get 40
                local.get 41
                i32.eq
                local.set 42
                i32.const 1
                local.set 43
                local.get 42
                local.get 43
                i32.and
                local.set 44
                local.get 44
                i32.eqz
                br_if 1 (;@5;)
              end
              local.get 3
              i32.load offset=8
              local.set 45
              local.get 3
              i32.load offset=4
              local.set 46
              local.get 45
              local.get 46
              i32.add
              local.set 47
              local.get 47
              i32.load8_u
              local.set 48
              i32.const 24
              local.set 49
              local.get 48
              local.get 49
              i32.shl
              local.set 50
              local.get 50
              local.get 49
              i32.shr_s
              local.set 51
              local.get 51
              call $push
              br 1 (;@4;)
            end
            i32.const 0
            local.set 52
            local.get 52
            i32.load offset=1148
            local.set 53
            i32.const 0
            local.set 54
            local.get 53
            local.get 54
            i32.le_s
            local.set 55
            i32.const 1
            local.set 56
            local.get 55
            local.get 56
            i32.and
            local.set 57
            block  ;; label = @5
              local.get 57
              i32.eqz
              br_if 0 (;@5;)
              i32.const 0
              local.set 58
              local.get 3
              local.get 58
              i32.store offset=12
              br 4 (;@1;)
            end
            call $pop
            local.set 59
            local.get 3
            local.get 59
            i32.store8 offset=3
            local.get 3
            i32.load8_u offset=3
            local.set 60
            i32.const 24
            local.set 61
            local.get 60
            local.get 61
            i32.shl
            local.set 62
            local.get 62
            local.get 61
            i32.shr_s
            local.set 63
            i32.const 123
            local.set 64
            local.get 63
            local.get 64
            i32.eq
            local.set 65
            i32.const 1
            local.set 66
            local.get 65
            local.get 66
            i32.and
            local.set 67
            block  ;; label = @5
              local.get 67
              i32.eqz
              br_if 0 (;@5;)
              local.get 3
              i32.load offset=8
              local.set 68
              local.get 3
              i32.load offset=4
              local.set 69
              local.get 68
              local.get 69
              i32.add
              local.set 70
              local.get 70
              i32.load8_u
              local.set 71
              i32.const 24
              local.set 72
              local.get 71
              local.get 72
              i32.shl
              local.set 73
              local.get 73
              local.get 72
              i32.shr_s
              local.set 74
              i32.const 125
              local.set 75
              local.get 74
              local.get 75
              i32.ne
              local.set 76
              i32.const 1
              local.set 77
              local.get 76
              local.get 77
              i32.and
              local.set 78
              local.get 78
              i32.eqz
              br_if 0 (;@5;)
              i32.const 0
              local.set 79
              local.get 3
              local.get 79
              i32.store offset=12
              br 4 (;@1;)
            end
            local.get 3
            i32.load8_u offset=3
            local.set 80
            i32.const 24
            local.set 81
            local.get 80
            local.get 81
            i32.shl
            local.set 82
            local.get 82
            local.get 81
            i32.shr_s
            local.set 83
            i32.const 91
            local.set 84
            local.get 83
            local.get 84
            i32.eq
            local.set 85
            i32.const 1
            local.set 86
            local.get 85
            local.get 86
            i32.and
            local.set 87
            block  ;; label = @5
              local.get 87
              i32.eqz
              br_if 0 (;@5;)
              local.get 3
              i32.load offset=8
              local.set 88
              local.get 3
              i32.load offset=4
              local.set 89
              local.get 88
              local.get 89
              i32.add
              local.set 90
              local.get 90
              i32.load8_u
              local.set 91
              i32.const 24
              local.set 92
              local.get 91
              local.get 92
              i32.shl
              local.set 93
              local.get 93
              local.get 92
              i32.shr_s
              local.set 94
              i32.const 93
              local.set 95
              local.get 94
              local.get 95
              i32.ne
              local.set 96
              i32.const 1
              local.set 97
              local.get 96
              local.get 97
              i32.and
              local.set 98
              local.get 98
              i32.eqz
              br_if 0 (;@5;)
              i32.const 0
              local.set 99
              local.get 3
              local.get 99
              i32.store offset=12
              br 4 (;@1;)
            end
            local.get 3
            i32.load8_u offset=3
            local.set 100
            i32.const 24
            local.set 101
            local.get 100
            local.get 101
            i32.shl
            local.set 102
            local.get 102
            local.get 101
            i32.shr_s
            local.set 103
            i32.const 40
            local.set 104
            local.get 103
            local.get 104
            i32.eq
            local.set 105
            i32.const 1
            local.set 106
            local.get 105
            local.get 106
            i32.and
            local.set 107
            block  ;; label = @5
              local.get 107
              i32.eqz
              br_if 0 (;@5;)
              local.get 3
              i32.load offset=8
              local.set 108
              local.get 3
              i32.load offset=4
              local.set 109
              local.get 108
              local.get 109
              i32.add
              local.set 110
              local.get 110
              i32.load8_u
              local.set 111
              i32.const 24
              local.set 112
              local.get 111
              local.get 112
              i32.shl
              local.set 113
              local.get 113
              local.get 112
              i32.shr_s
              local.set 114
              i32.const 41
              local.set 115
              local.get 114
              local.get 115
              i32.ne
              local.set 116
              i32.const 1
              local.set 117
              local.get 116
              local.get 117
              i32.and
              local.set 118
              local.get 118
              i32.eqz
              br_if 0 (;@5;)
              i32.const 0
              local.set 119
              local.get 3
              local.get 119
              i32.store offset=12
              br 4 (;@1;)
            end
          end
          local.get 3
          i32.load offset=4
          local.set 120
          i32.const 1
          local.set 121
          local.get 120
          local.get 121
          i32.add
          local.set 122
          local.get 3
          local.get 122
          i32.store offset=4
          br 0 (;@3;)
        end
      end
      i32.const 0
      local.set 123
      local.get 123
      i32.load offset=1148
      local.set 124
      i32.const 1
      local.set 125
      i32.const 0
      local.set 126
      local.get 126
      local.get 125
      local.get 124
      select
      local.set 127
      local.get 3
      local.get 127
      i32.store offset=12
    end
    local.get 3
    i32.load offset=12
    local.set 128
    i32.const 16
    local.set 129
    local.get 3
    local.get 129
    i32.add
    local.set 130
    local.get 130
    global.set $__stack_pointer
    local.get 128
    return)
  (func $destroyStack (type 3)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 16
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 3
    i32.load offset=1152
    local.set 4
    local.get 2
    local.get 4
    i32.store offset=12
    i32.const 0
    local.set 5
    local.get 5
    i32.load offset=1148
    local.set 6
    i32.const 0
    local.set 7
    local.get 6
    local.get 7
    i32.gt_s
    local.set 8
    i32.const 1
    local.set 9
    local.get 8
    local.get 9
    i32.and
    local.set 10
    block  ;; label = @1
      local.get 10
      i32.eqz
      br_if 0 (;@1;)
      block  ;; label = @2
        loop  ;; label = @3
          local.get 2
          i32.load offset=12
          local.set 11
          local.get 11
          i32.load offset=4
          local.set 12
          i32.const 0
          local.set 13
          local.get 12
          local.get 13
          i32.ne
          local.set 14
          i32.const 1
          local.set 15
          local.get 14
          local.get 15
          i32.and
          local.set 16
          local.get 16
          i32.eqz
          br_if 1 (;@2;)
          local.get 2
          i32.load offset=12
          local.set 17
          local.get 2
          local.get 17
          i32.store offset=8
          local.get 2
          i32.load offset=12
          local.set 18
          local.get 18
          i32.load offset=4
          local.set 19
          local.get 2
          local.get 19
          i32.store offset=12
          local.get 2
          i32.load offset=8
          local.set 20
          local.get 20
          call $free
          br 0 (;@3;)
        end
      end
      i32.const 0
      local.set 21
      i32.const 0
      local.set 22
      local.get 22
      local.get 21
      i32.store offset=1148
    end
    i32.const 16
    local.set 23
    local.get 2
    local.get 23
    i32.add
    local.set 24
    local.get 24
    global.set $__stack_pointer
    return)
  (func $__original_main (type 4) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 144
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 2
    local.get 3
    i32.store offset=140
    i32.const 1120
    local.set 4
    i32.const 0
    local.set 5
    local.get 4
    local.get 5
    call $printf
    drop
    i32.const 1030
    local.set 6
    i32.const 0
    local.set 7
    local.get 6
    local.get 7
    call $printf
    drop
    i32.const 136
    local.set 8
    local.get 2
    local.get 8
    i32.add
    local.set 9
    local.get 2
    local.get 9
    i32.store offset=16
    i32.const 1027
    local.set 10
    i32.const 16
    local.set 11
    local.get 2
    local.get 11
    i32.add
    local.set 12
    local.get 10
    local.get 12
    call $scanf
    drop
    i32.const 0
    local.set 13
    local.get 2
    local.get 13
    i32.store offset=132
    block  ;; label = @1
      loop  ;; label = @2
        local.get 2
        i32.load offset=132
        local.set 14
        local.get 2
        i32.load offset=136
        local.set 15
        local.get 14
        local.get 15
        i32.lt_s
        local.set 16
        i32.const 1
        local.set 17
        local.get 16
        local.get 17
        i32.and
        local.set 18
        local.get 18
        i32.eqz
        br_if 1 (;@1;)
        i32.const 1078
        local.set 19
        i32.const 0
        local.set 20
        local.get 19
        local.get 20
        call $printf
        drop
        i32.const 32
        local.set 21
        local.get 2
        local.get 21
        i32.add
        local.set 22
        local.get 22
        local.set 23
        local.get 2
        local.get 23
        i32.store
        i32.const 1024
        local.set 24
        local.get 24
        local.get 2
        call $scanf
        drop
        i32.const 32
        local.set 25
        local.get 2
        local.get 25
        i32.add
        local.set 26
        local.get 26
        local.set 27
        local.get 27
        call $isBalanced
        local.set 28
        block  ;; label = @3
          block  ;; label = @4
            local.get 28
            i32.eqz
            br_if 0 (;@4;)
            i32.const 1109
            local.set 29
            i32.const 0
            local.set 30
            local.get 29
            local.get 30
            call $printf
            drop
            br 1 (;@3;)
          end
          i32.const 1115
          local.set 31
          i32.const 0
          local.set 32
          local.get 31
          local.get 32
          call $printf
          drop
        end
        call $destroyStack
        local.get 2
        i32.load offset=132
        local.set 33
        i32.const 1
        local.set 34
        local.get 33
        local.get 34
        i32.add
        local.set 35
        local.get 2
        local.get 35
        i32.store offset=132
        br 0 (;@2;)
      end
    end
    i32.const 0
    local.set 36
    i32.const 144
    local.set 37
    local.get 2
    local.get 37
    i32.add
    local.set 38
    local.get 38
    global.set $__stack_pointer
    local.get 36
    return)
  (func $main (type 2) (param i32 i32) (result i32)
    (local i32)
    call $__original_main
    local.set 2
    local.get 2
    return)
  (table (;0;) 1 1 funcref)
  (memory (;0;) 2)
  (global $__stack_pointer (mut i32) (i32.const 66704))
  (global (;1;) i32 (i32.const 1152))
  (global (;2;) i32 (i32.const 1148))
  (global (;3;) i32 (i32.const 1024))
  (global (;4;) i32 (i32.const 1156))
  (global (;5;) i32 (i32.const 1168))
  (global (;6;) i32 (i32.const 66704))
  (global (;7;) i32 (i32.const 1024))
  (global (;8;) i32 (i32.const 66704))
  (global (;9;) i32 (i32.const 131072))
  (global (;10;) i32 (i32.const 0))
  (global (;11;) i32 (i32.const 1))
  (export "memory" (memory 0))
  (export "__wasm_call_ctors" (func $__wasm_call_ctors))
  (export "push" (func $push))
  (export "head" (global 1))
  (export "c" (global 2))
  (export "pop" (func $pop))
  (export "isBalanced" (func $isBalanced))
  (export "destroyStack" (func $destroyStack))
  (export "__original_main" (func $__original_main))
  (export "main" (func $main))
  (export "__main_void" (func $__original_main))
  (export "__indirect_function_table" (table 0))
  (export "__dso_handle" (global 3))
  (export "__data_end" (global 4))
  (export "__stack_low" (global 5))
  (export "__stack_high" (global 6))
  (export "__global_base" (global 7))
  (export "__heap_base" (global 8))
  (export "__heap_end" (global 9))
  (export "__memory_base" (global 10))
  (export "__table_base" (global 11))
  (data $.rodata (i32.const 1024) "%s\00%d\00\0aPlease enter the number of processing rounds? \00\0aPlease enter the expression? \00\0aYES\0a\00\0aNO\0a\00\09\09Balanced parenthesis\0a\0a\00"))
