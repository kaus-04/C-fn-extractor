(module $int_to_string.wasm
  (type (;0;) (func (param i32) (result i64)))
  (type (;1;) (func (param i32)))
  (type (;2;) (func (param i32 i32) (result i32)))
  (type (;3;) (func (result i32)))
  (type (;4;) (func (param i32 i32 i32 i32) (result i32)))
  (type (;5;) (func (param i32 i32 i32 i32)))
  (type (;6;) (func))
  (type (;7;) (func (param i32 i32 i32) (result i32)))
  (import "env" "time" (func $time (type 0)))
  (import "env" "srand" (func $srand (type 1)))
  (import "env" "calloc" (func $calloc (type 2)))
  (import "env" "rand" (func $rand (type 3)))
  (import "env" "snprintf" (func $snprintf (type 4)))
  (import "env" "strcmp" (func $strcmp (type 2)))
  (import "env" "__assert_fail" (func $__assert_fail (type 5)))
  (import "env" "free" (func $free (type 1)))
  (func $__wasm_call_ctors (type 6))
  (func $int_to_string (type 7) (param i32 i32 i32) (result i32)
    (local i32 i32 i32 i32 i64 i64 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 3
    i32.const 48
    local.set 4
    local.get 3
    local.get 4
    i32.sub
    local.set 5
    local.get 5
    local.get 0
    i32.store16 offset=46
    local.get 5
    local.get 1
    i32.store offset=40
    local.get 5
    local.get 2
    i32.store offset=36
    i32.const 0
    local.set 6
    local.get 6
    i64.load offset=1032
    local.set 7
    local.get 5
    local.get 7
    i64.store offset=24
    local.get 6
    i64.load offset=1024
    local.set 8
    local.get 5
    local.get 8
    i64.store offset=16
    i32.const 0
    local.set 9
    local.get 5
    local.get 9
    i32.store offset=12
    loop  ;; label = @1
      local.get 5
      i32.load16_u offset=46
      local.set 10
      i32.const 65535
      local.set 11
      local.get 10
      local.get 11
      i32.and
      local.set 12
      local.get 5
      i32.load offset=36
      local.set 13
      local.get 12
      local.get 13
      i32.rem_s
      local.set 14
      i32.const 16
      local.set 15
      local.get 5
      local.get 15
      i32.add
      local.set 16
      local.get 16
      local.set 17
      local.get 17
      local.get 14
      i32.add
      local.set 18
      local.get 18
      i32.load8_u
      local.set 19
      local.get 5
      i32.load offset=40
      local.set 20
      local.get 5
      i32.load offset=12
      local.set 21
      i32.const 1
      local.set 22
      local.get 21
      local.get 22
      i32.add
      local.set 23
      local.get 5
      local.get 23
      i32.store offset=12
      local.get 20
      local.get 21
      i32.add
      local.set 24
      local.get 24
      local.get 19
      i32.store8
      local.get 5
      i32.load offset=36
      local.set 25
      local.get 5
      i32.load16_u offset=46
      local.set 26
      i32.const 65535
      local.set 27
      local.get 26
      local.get 27
      i32.and
      local.set 28
      local.get 28
      local.get 25
      i32.div_s
      local.set 29
      local.get 5
      local.get 29
      i32.store16 offset=46
      local.get 5
      i32.load16_u offset=46
      local.set 30
      i32.const 65535
      local.set 31
      local.get 30
      local.get 31
      i32.and
      local.set 32
      local.get 32
      br_if 0 (;@1;)
    end
    i32.const 0
    local.set 33
    local.get 5
    local.get 33
    i32.store offset=8
    local.get 5
    i32.load offset=12
    local.set 34
    i32.const 2
    local.set 35
    local.get 34
    local.get 35
    i32.div_s
    local.set 36
    local.get 5
    local.get 36
    i32.store offset=4
    block  ;; label = @1
      loop  ;; label = @2
        local.get 5
        i32.load offset=8
        local.set 37
        local.get 5
        i32.load offset=4
        local.set 38
        local.get 37
        local.get 38
        i32.lt_s
        local.set 39
        i32.const 1
        local.set 40
        local.get 39
        local.get 40
        i32.and
        local.set 41
        local.get 41
        i32.eqz
        br_if 1 (;@1;)
        local.get 5
        i32.load offset=40
        local.set 42
        local.get 5
        i32.load offset=8
        local.set 43
        local.get 42
        local.get 43
        i32.add
        local.set 44
        local.get 44
        i32.load8_u
        local.set 45
        local.get 5
        local.get 45
        i32.store8 offset=3
        local.get 5
        i32.load offset=40
        local.set 46
        local.get 5
        i32.load offset=12
        local.set 47
        i32.const 1
        local.set 48
        local.get 47
        local.get 48
        i32.sub
        local.set 49
        local.get 5
        i32.load offset=8
        local.set 50
        local.get 49
        local.get 50
        i32.sub
        local.set 51
        local.get 46
        local.get 51
        i32.add
        local.set 52
        local.get 52
        i32.load8_u
        local.set 53
        local.get 5
        i32.load offset=40
        local.set 54
        local.get 5
        i32.load offset=8
        local.set 55
        local.get 54
        local.get 55
        i32.add
        local.set 56
        local.get 56
        local.get 53
        i32.store8
        local.get 5
        i32.load8_u offset=3
        local.set 57
        local.get 5
        i32.load offset=40
        local.set 58
        local.get 5
        i32.load offset=12
        local.set 59
        i32.const 1
        local.set 60
        local.get 59
        local.get 60
        i32.sub
        local.set 61
        local.get 5
        i32.load offset=8
        local.set 62
        local.get 61
        local.get 62
        i32.sub
        local.set 63
        local.get 58
        local.get 63
        i32.add
        local.set 64
        local.get 64
        local.get 57
        i32.store8
        local.get 5
        i32.load offset=8
        local.set 65
        i32.const 1
        local.set 66
        local.get 65
        local.get 66
        i32.add
        local.set 67
        local.get 5
        local.get 67
        i32.store offset=8
        br 0 (;@2;)
      end
    end
    local.get 5
    i32.load offset=40
    local.set 68
    local.get 5
    i32.load offset=12
    local.set 69
    local.get 68
    local.get 69
    i32.add
    local.set 70
    i32.const 0
    local.set 71
    local.get 70
    local.get 71
    i32.store8
    local.get 5
    i32.load offset=40
    local.set 72
    local.get 72
    return)
  (func $__original_main (type 3) (result i32)
    (local i32 i32 i32 i32 i32 i64 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 16
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 2
    local.get 3
    i32.store offset=12
    i32.const 0
    local.set 4
    local.get 4
    call $time
    local.set 5
    local.get 5
    i32.wrap_i64
    local.set 6
    local.get 6
    call $srand
    call $test
    i32.const 0
    local.set 7
    i32.const 16
    local.set 8
    local.get 2
    local.get 8
    i32.add
    local.set 9
    local.get 9
    global.set $__stack_pointer
    local.get 7
    return)
  (func $test (type 6)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 64
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 100
    local.set 3
    local.get 2
    local.get 3
    i32.store offset=60
    i32.const 1
    local.set 4
    i32.const 100
    local.set 5
    local.get 4
    local.get 5
    call $calloc
    local.set 6
    local.get 2
    local.get 6
    i32.store offset=56
    i32.const 1
    local.set 7
    i32.const 100
    local.set 8
    local.get 7
    local.get 8
    call $calloc
    local.set 9
    local.get 2
    local.get 9
    i32.store offset=52
    i32.const 1
    local.set 10
    local.get 2
    local.get 10
    i32.store offset=48
    block  ;; label = @1
      loop  ;; label = @2
        local.get 2
        i32.load offset=48
        local.set 11
        i32.const 100
        local.set 12
        local.get 11
        local.get 12
        i32.le_s
        local.set 13
        i32.const 1
        local.set 14
        local.get 13
        local.get 14
        i32.and
        local.set 15
        local.get 15
        i32.eqz
        br_if 1 (;@1;)
        call $rand
        local.set 16
        i32.const 100
        local.set 17
        local.get 16
        local.get 17
        i32.rem_s
        local.set 18
        local.get 2
        local.get 18
        i32.store offset=44
        local.get 2
        i32.load offset=56
        local.set 19
        local.get 2
        i32.load offset=44
        local.set 20
        local.get 2
        local.get 20
        i32.store offset=32
        i32.const 1048
        local.set 21
        i32.const 100
        local.set 22
        i32.const 32
        local.set 23
        local.get 2
        local.get 23
        i32.add
        local.set 24
        local.get 19
        local.get 22
        local.get 21
        local.get 24
        call $snprintf
        drop
        local.get 2
        i32.load offset=56
        local.set 25
        local.get 2
        i32.load offset=44
        local.set 26
        local.get 2
        i32.load offset=52
        local.set 27
        i32.const 8
        local.set 28
        i32.const 65535
        local.set 29
        local.get 26
        local.get 29
        i32.and
        local.set 30
        local.get 30
        local.get 27
        local.get 28
        call $int_to_string
        local.set 31
        local.get 25
        local.get 31
        call $strcmp
        local.set 32
        block  ;; label = @3
          local.get 32
          i32.eqz
          br_if 0 (;@3;)
          i32.const 1095
          local.set 33
          i32.const 1054
          local.set 34
          i32.const 65
          local.set 35
          i32.const 1043
          local.set 36
          local.get 33
          local.get 34
          local.get 35
          local.get 36
          call $__assert_fail
          unreachable
        end
        local.get 2
        i32.load offset=56
        local.set 37
        local.get 2
        i32.load offset=44
        local.set 38
        local.get 2
        local.get 38
        i32.store offset=16
        i32.const 1051
        local.set 39
        i32.const 100
        local.set 40
        i32.const 16
        local.set 41
        local.get 2
        local.get 41
        i32.add
        local.set 42
        local.get 37
        local.get 40
        local.get 39
        local.get 42
        call $snprintf
        drop
        local.get 2
        i32.load offset=56
        local.set 43
        local.get 2
        i32.load offset=44
        local.set 44
        local.get 2
        i32.load offset=52
        local.set 45
        i32.const 10
        local.set 46
        i32.const 65535
        local.set 47
        local.get 44
        local.get 47
        i32.and
        local.set 48
        local.get 48
        local.get 45
        local.get 46
        call $int_to_string
        local.set 49
        local.get 43
        local.get 49
        call $strcmp
        local.set 50
        block  ;; label = @3
          local.get 50
          i32.eqz
          br_if 0 (;@3;)
          i32.const 1194
          local.set 51
          i32.const 1054
          local.set 52
          i32.const 67
          local.set 53
          i32.const 1043
          local.set 54
          local.get 51
          local.get 52
          local.get 53
          local.get 54
          call $__assert_fail
          unreachable
        end
        local.get 2
        i32.load offset=56
        local.set 55
        local.get 2
        i32.load offset=44
        local.set 56
        local.get 2
        local.get 56
        i32.store
        i32.const 1040
        local.set 57
        i32.const 100
        local.set 58
        local.get 55
        local.get 58
        local.get 57
        local.get 2
        call $snprintf
        drop
        local.get 2
        i32.load offset=56
        local.set 59
        local.get 2
        i32.load offset=44
        local.set 60
        local.get 2
        i32.load offset=52
        local.set 61
        i32.const 16
        local.set 62
        i32.const 65535
        local.set 63
        local.get 60
        local.get 63
        i32.and
        local.set 64
        local.get 64
        local.get 61
        local.get 62
        call $int_to_string
        local.set 65
        local.get 59
        local.get 65
        call $strcmp
        local.set 66
        block  ;; label = @3
          local.get 66
          i32.eqz
          br_if 0 (;@3;)
          i32.const 1144
          local.set 67
          i32.const 1054
          local.set 68
          i32.const 69
          local.set 69
          i32.const 1043
          local.set 70
          local.get 67
          local.get 68
          local.get 69
          local.get 70
          call $__assert_fail
          unreachable
        end
        local.get 2
        i32.load offset=48
        local.set 71
        i32.const 1
        local.set 72
        local.get 71
        local.get 72
        i32.add
        local.set 73
        local.get 2
        local.get 73
        i32.store offset=48
        br 0 (;@2;)
      end
    end
    local.get 2
    i32.load offset=56
    local.set 74
    local.get 74
    call $free
    local.get 2
    i32.load offset=52
    local.set 75
    local.get 75
    call $free
    i32.const 64
    local.set 76
    local.get 2
    local.get 76
    i32.add
    local.set 77
    local.get 77
    global.set $__stack_pointer
    return)
  (func $main (type 2) (param i32 i32) (result i32)
    (local i32)
    call $__original_main
    local.set 2
    local.get 2
    return)
  (table (;0;) 1 1 funcref)
  (memory (;0;) 2)
  (global $__stack_pointer (mut i32) (i32.const 66784))
  (global (;1;) i32 (i32.const 1024))
  (global (;2;) i32 (i32.const 1244))
  (global (;3;) i32 (i32.const 1248))
  (global (;4;) i32 (i32.const 66784))
  (global (;5;) i32 (i32.const 1024))
  (global (;6;) i32 (i32.const 66784))
  (global (;7;) i32 (i32.const 131072))
  (global (;8;) i32 (i32.const 0))
  (global (;9;) i32 (i32.const 1))
  (export "memory" (memory 0))
  (export "__wasm_call_ctors" (func $__wasm_call_ctors))
  (export "int_to_string" (func $int_to_string))
  (export "__original_main" (func $__original_main))
  (export "main" (func $main))
  (export "__main_void" (func $__original_main))
  (export "__indirect_function_table" (table 0))
  (export "__dso_handle" (global 1))
  (export "__data_end" (global 2))
  (export "__stack_low" (global 3))
  (export "__stack_high" (global 4))
  (export "__global_base" (global 5))
  (export "__heap_base" (global 6))
  (export "__heap_end" (global 7))
  (export "__memory_base" (global 8))
  (export "__table_base" (global 9))
  (data $.rodata (i32.const 1024) "0123456789abcdef%x\00test\00%o\00%d\00../files/pop/conversions/int_to_string.c\00strcmp(str1, int_to_string(value, str2, 8)) == 0\00strcmp(str1, int_to_string(value, str2, 16)) == 0\00strcmp(str1, int_to_string(value, str2, 10)) == 0\00"))
