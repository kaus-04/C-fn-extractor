(module $postfix_evaluation.wasm
  (type (;0;) (func (param i32 i32) (result i32)))
  (type (;1;) (func (param i32)))
  (type (;2;) (func (param i32) (result i32)))
  (type (;3;) (func (param i32 i32 i32 i32)))
  (type (;4;) (func))
  (type (;5;) (func (result i32)))
  (import "env" "printf" (func $printf (type 0)))
  (import "env" "exit" (func $exit (type 1)))
  (import "env" "strlen" (func $strlen (type 2)))
  (import "env" "isdigit" (func $isdigit (type 2)))
  (import "env" "__assert_fail" (func $__assert_fail (type 3)))
  (func $__wasm_call_ctors (type 4))
  (func $push (type 1) (param i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 1
    i32.const 16
    local.set 2
    local.get 1
    local.get 2
    i32.sub
    local.set 3
    local.get 3
    global.set $__stack_pointer
    local.get 3
    local.get 0
    i32.store8 offset=15
    i32.const 0
    local.set 4
    local.get 4
    i32.load offset=1352
    local.set 5
    i32.const 19
    local.set 6
    local.get 5
    local.get 6
    i32.eq
    local.set 7
    i32.const 1
    local.set 8
    local.get 7
    local.get 8
    i32.and
    local.set 9
    block  ;; label = @1
      local.get 9
      i32.eqz
      br_if 0 (;@1;)
      i32.const 1128
      local.set 10
      i32.const 0
      local.set 11
      local.get 10
      local.get 11
      call $printf
      drop
      i32.const 1
      local.set 12
      local.get 12
      call $exit
      unreachable
    end
    i32.const 0
    local.set 13
    local.get 13
    i32.load offset=1352
    local.set 14
    i32.const 1
    local.set 15
    local.get 14
    local.get 15
    i32.add
    local.set 16
    i32.const 0
    local.set 17
    local.get 17
    local.get 16
    i32.store offset=1352
    local.get 3
    i32.load8_u offset=15
    local.set 18
    i32.const 0
    local.set 19
    local.get 19
    i32.load offset=1352
    local.set 20
    local.get 20
    local.get 18
    i32.store8 offset=1332
    i32.const 16
    local.set 21
    local.get 3
    local.get 21
    i32.add
    local.set 22
    local.get 22
    global.set $__stack_pointer
    return)
  (func $pop (type 5) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 16
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 3
    i32.load offset=1352
    local.set 4
    i32.const -1
    local.set 5
    local.get 4
    local.get 5
    i32.eq
    local.set 6
    i32.const 1
    local.set 7
    local.get 6
    local.get 7
    i32.and
    local.set 8
    block  ;; label = @1
      local.get 8
      i32.eqz
      br_if 0 (;@1;)
      i32.const 1146
      local.set 9
      i32.const 0
      local.set 10
      local.get 9
      local.get 10
      call $printf
      drop
      i32.const 1
      local.set 11
      local.get 11
      call $exit
      unreachable
    end
    i32.const 0
    local.set 12
    local.get 12
    i32.load offset=1352
    local.set 13
    local.get 13
    i32.load8_u offset=1332
    local.set 14
    local.get 2
    local.get 14
    i32.store8 offset=15
    i32.const 0
    local.set 15
    local.get 15
    i32.load offset=1352
    local.set 16
    i32.const -1
    local.set 17
    local.get 16
    local.get 17
    i32.add
    local.set 18
    i32.const 0
    local.set 19
    local.get 19
    local.get 18
    i32.store offset=1352
    local.get 2
    i32.load8_u offset=15
    local.set 20
    i32.const 24
    local.set 21
    local.get 20
    local.get 21
    i32.shl
    local.set 22
    local.get 22
    local.get 21
    i32.shr_s
    local.set 23
    i32.const 16
    local.set 24
    local.get 2
    local.get 24
    i32.add
    local.set 25
    local.get 25
    global.set $__stack_pointer
    local.get 23
    return)
  (func $evaluate (type 2) (param i32) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 1
    i32.const 16
    local.set 2
    local.get 1
    local.get 2
    i32.sub
    local.set 3
    local.get 3
    global.set $__stack_pointer
    local.get 3
    local.get 0
    i32.store offset=12
    i32.const 0
    local.set 4
    local.get 3
    local.get 4
    i32.store offset=4
    block  ;; label = @1
      loop  ;; label = @2
        local.get 3
        i32.load offset=4
        local.set 5
        local.get 3
        i32.load offset=12
        local.set 6
        local.get 6
        call $strlen
        local.set 7
        local.get 5
        local.get 7
        i32.lt_u
        local.set 8
        i32.const 1
        local.set 9
        local.get 8
        local.get 9
        i32.and
        local.set 10
        local.get 10
        i32.eqz
        br_if 1 (;@1;)
        local.get 3
        i32.load offset=12
        local.set 11
        local.get 3
        i32.load offset=4
        local.set 12
        local.get 11
        local.get 12
        i32.add
        local.set 13
        local.get 13
        i32.load8_u
        local.set 14
        i32.const 24
        local.set 15
        local.get 14
        local.get 15
        i32.shl
        local.set 16
        local.get 16
        local.get 15
        i32.shr_s
        local.set 17
        i32.const 32
        local.set 18
        local.get 17
        local.get 18
        i32.eq
        local.set 19
        i32.const 1
        local.set 20
        local.get 19
        local.get 20
        i32.and
        local.set 21
        block  ;; label = @3
          block  ;; label = @4
            local.get 21
            i32.eqz
            br_if 0 (;@4;)
            br 1 (;@3;)
          end
          i32.const 0
          local.set 22
          i32.const 1
          local.set 23
          local.get 22
          local.get 23
          i32.and
          local.set 24
          block  ;; label = @4
            block  ;; label = @5
              block  ;; label = @6
                block  ;; label = @7
                  local.get 24
                  i32.eqz
                  br_if 0 (;@7;)
                  local.get 3
                  i32.load offset=12
                  local.set 25
                  local.get 3
                  i32.load offset=4
                  local.set 26
                  local.get 25
                  local.get 26
                  i32.add
                  local.set 27
                  local.get 27
                  i32.load8_u
                  local.set 28
                  i32.const 24
                  local.set 29
                  local.get 28
                  local.get 29
                  i32.shl
                  local.set 30
                  local.get 30
                  local.get 29
                  i32.shr_s
                  local.set 31
                  local.get 31
                  call $isdigit
                  local.set 32
                  local.get 32
                  br_if 1 (;@6;)
                  br 2 (;@5;)
                end
                local.get 3
                i32.load offset=12
                local.set 33
                local.get 3
                i32.load offset=4
                local.set 34
                local.get 33
                local.get 34
                i32.add
                local.set 35
                local.get 35
                i32.load8_u
                local.set 36
                i32.const 24
                local.set 37
                local.get 36
                local.get 37
                i32.shl
                local.set 38
                local.get 38
                local.get 37
                i32.shr_s
                local.set 39
                i32.const 48
                local.set 40
                local.get 39
                local.get 40
                i32.sub
                local.set 41
                i32.const 10
                local.set 42
                local.get 41
                local.get 42
                i32.lt_u
                local.set 43
                i32.const 1
                local.set 44
                local.get 43
                local.get 44
                i32.and
                local.set 45
                local.get 45
                i32.eqz
                br_if 1 (;@5;)
              end
              i32.const 0
              local.set 46
              local.get 3
              local.get 46
              i32.store8 offset=8
              loop  ;; label = @6
                local.get 3
                i32.load8_u offset=8
                local.set 47
                i32.const 24
                local.set 48
                local.get 47
                local.get 48
                i32.shl
                local.set 49
                local.get 49
                local.get 48
                i32.shr_s
                local.set 50
                i32.const 10
                local.set 51
                local.get 50
                local.get 51
                i32.mul
                local.set 52
                local.get 3
                i32.load offset=12
                local.set 53
                local.get 3
                i32.load offset=4
                local.set 54
                local.get 53
                local.get 54
                i32.add
                local.set 55
                local.get 55
                i32.load8_u
                local.set 56
                i32.const 24
                local.set 57
                local.get 56
                local.get 57
                i32.shl
                local.set 58
                local.get 58
                local.get 57
                i32.shr_s
                local.set 59
                i32.const 48
                local.set 60
                local.get 59
                local.get 60
                i32.sub
                local.set 61
                local.get 52
                local.get 61
                i32.add
                local.set 62
                local.get 3
                local.get 62
                i32.store8 offset=8
                local.get 3
                i32.load offset=4
                local.set 63
                i32.const 1
                local.set 64
                local.get 63
                local.get 64
                i32.add
                local.set 65
                local.get 3
                local.get 65
                i32.store offset=4
                local.get 3
                i32.load offset=4
                local.set 66
                local.get 3
                i32.load offset=12
                local.set 67
                local.get 67
                call $strlen
                local.set 68
                local.get 66
                local.get 68
                i32.lt_u
                local.set 69
                i32.const 0
                local.set 70
                i32.const 1
                local.set 71
                local.get 69
                local.get 71
                i32.and
                local.set 72
                local.get 70
                local.set 73
                block  ;; label = @7
                  local.get 72
                  i32.eqz
                  br_if 0 (;@7;)
                  local.get 3
                  i32.load offset=12
                  local.set 74
                  local.get 3
                  i32.load offset=4
                  local.set 75
                  local.get 74
                  local.get 75
                  i32.add
                  local.set 76
                  local.get 76
                  i32.load8_u
                  local.set 77
                  i32.const 24
                  local.set 78
                  local.get 77
                  local.get 78
                  i32.shl
                  local.set 79
                  local.get 79
                  local.get 78
                  i32.shr_s
                  local.set 80
                  i32.const 48
                  local.set 81
                  local.get 80
                  local.get 81
                  i32.sub
                  local.set 82
                  i32.const 10
                  local.set 83
                  local.get 82
                  local.get 83
                  i32.lt_u
                  local.set 84
                  local.get 84
                  local.set 73
                end
                local.get 73
                local.set 85
                i32.const 1
                local.set 86
                local.get 85
                local.get 86
                i32.and
                local.set 87
                local.get 87
                br_if 0 (;@6;)
              end
              local.get 3
              i32.load8_u offset=8
              local.set 88
              i32.const 24
              local.set 89
              local.get 88
              local.get 89
              i32.shl
              local.set 90
              local.get 90
              local.get 89
              i32.shr_s
              local.set 91
              local.get 91
              call $push
              br 1 (;@4;)
            end
            call $pop
            local.set 92
            local.get 3
            local.get 92
            i32.store8 offset=10
            call $pop
            local.set 93
            local.get 3
            local.get 93
            i32.store8 offset=11
            local.get 3
            i32.load offset=12
            local.set 94
            local.get 3
            i32.load offset=4
            local.set 95
            local.get 94
            local.get 95
            i32.add
            local.set 96
            local.get 96
            i32.load8_s
            local.set 97
            i32.const -37
            local.set 98
            local.get 97
            local.get 98
            i32.add
            local.set 99
            i32.const 10
            local.set 100
            local.get 99
            local.get 100
            i32.gt_u
            drop
            block  ;; label = @5
              block  ;; label = @6
                block  ;; label = @7
                  block  ;; label = @8
                    block  ;; label = @9
                      block  ;; label = @10
                        block  ;; label = @11
                          local.get 99
                          br_table 4 (;@7;) 5 (;@6;) 5 (;@6;) 5 (;@6;) 5 (;@6;) 2 (;@9;) 0 (;@11;) 5 (;@6;) 1 (;@10;) 5 (;@6;) 3 (;@8;) 5 (;@6;)
                        end
                        local.get 3
                        i32.load8_u offset=11
                        local.set 101
                        i32.const 24
                        local.set 102
                        local.get 101
                        local.get 102
                        i32.shl
                        local.set 103
                        local.get 103
                        local.get 102
                        i32.shr_s
                        local.set 104
                        local.get 3
                        i32.load8_u offset=10
                        local.set 105
                        i32.const 24
                        local.set 106
                        local.get 105
                        local.get 106
                        i32.shl
                        local.set 107
                        local.get 107
                        local.get 106
                        i32.shr_s
                        local.set 108
                        local.get 104
                        local.get 108
                        i32.add
                        local.set 109
                        local.get 3
                        local.get 109
                        i32.store8 offset=9
                        br 5 (;@5;)
                      end
                      local.get 3
                      i32.load8_u offset=11
                      local.set 110
                      i32.const 24
                      local.set 111
                      local.get 110
                      local.get 111
                      i32.shl
                      local.set 112
                      local.get 112
                      local.get 111
                      i32.shr_s
                      local.set 113
                      local.get 3
                      i32.load8_u offset=10
                      local.set 114
                      i32.const 24
                      local.set 115
                      local.get 114
                      local.get 115
                      i32.shl
                      local.set 116
                      local.get 116
                      local.get 115
                      i32.shr_s
                      local.set 117
                      local.get 113
                      local.get 117
                      i32.sub
                      local.set 118
                      local.get 3
                      local.get 118
                      i32.store8 offset=9
                      br 4 (;@5;)
                    end
                    local.get 3
                    i32.load8_u offset=11
                    local.set 119
                    i32.const 24
                    local.set 120
                    local.get 119
                    local.get 120
                    i32.shl
                    local.set 121
                    local.get 121
                    local.get 120
                    i32.shr_s
                    local.set 122
                    local.get 3
                    i32.load8_u offset=10
                    local.set 123
                    i32.const 24
                    local.set 124
                    local.get 123
                    local.get 124
                    i32.shl
                    local.set 125
                    local.get 125
                    local.get 124
                    i32.shr_s
                    local.set 126
                    local.get 122
                    local.get 126
                    i32.mul
                    local.set 127
                    local.get 3
                    local.get 127
                    i32.store8 offset=9
                    br 3 (;@5;)
                  end
                  local.get 3
                  i32.load8_u offset=11
                  local.set 128
                  i32.const 24
                  local.set 129
                  local.get 128
                  local.get 129
                  i32.shl
                  local.set 130
                  local.get 130
                  local.get 129
                  i32.shr_s
                  local.set 131
                  local.get 3
                  i32.load8_u offset=10
                  local.set 132
                  i32.const 24
                  local.set 133
                  local.get 132
                  local.get 133
                  i32.shl
                  local.set 134
                  local.get 134
                  local.get 133
                  i32.shr_s
                  local.set 135
                  local.get 131
                  local.get 135
                  i32.div_s
                  local.set 136
                  local.get 3
                  local.get 136
                  i32.store8 offset=9
                  br 2 (;@5;)
                end
                local.get 3
                i32.load8_u offset=11
                local.set 137
                i32.const 24
                local.set 138
                local.get 137
                local.get 138
                i32.shl
                local.set 139
                local.get 139
                local.get 138
                i32.shr_s
                local.set 140
                local.get 3
                i32.load8_u offset=10
                local.set 141
                i32.const 24
                local.set 142
                local.get 141
                local.get 142
                i32.shl
                local.set 143
                local.get 143
                local.get 142
                i32.shr_s
                local.set 144
                local.get 140
                local.get 144
                i32.rem_s
                local.set 145
                local.get 3
                local.get 145
                i32.store8 offset=9
                br 1 (;@5;)
              end
              i32.const 1029
              local.set 146
              i32.const 0
              local.set 147
              local.get 146
              local.get 147
              call $printf
              drop
              i32.const 1
              local.set 148
              local.get 148
              call $exit
              unreachable
            end
            local.get 3
            i32.load8_u offset=9
            local.set 149
            i32.const 24
            local.set 150
            local.get 149
            local.get 150
            i32.shl
            local.set 151
            local.get 151
            local.get 150
            i32.shr_s
            local.set 152
            local.get 152
            call $push
          end
        end
        local.get 3
        i32.load offset=4
        local.set 153
        i32.const 1
        local.set 154
        local.get 153
        local.get 154
        i32.add
        local.set 155
        local.get 3
        local.get 155
        i32.store offset=4
        br 0 (;@2;)
      end
    end
    call $pop
    local.set 156
    i32.const 24
    local.set 157
    local.get 156
    local.get 157
    i32.shl
    local.set 158
    local.get 158
    local.get 157
    i32.shr_s
    local.set 159
    i32.const 16
    local.set 160
    local.get 3
    local.get 160
    i32.add
    local.set 161
    local.get 161
    global.set $__stack_pointer
    local.get 159
    return)
  (func $__original_main (type 5) (result i32)
    (local i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 16
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 2
    local.get 3
    i32.store offset=12
    i32.const -1
    local.set 4
    i32.const 0
    local.set 5
    local.get 5
    local.get 4
    i32.store offset=1352
    call $test
    i32.const 0
    local.set 6
    i32.const 16
    local.set 7
    local.get 2
    local.get 7
    i32.add
    local.set 8
    local.get 8
    global.set $__stack_pointer
    local.get 6
    return)
  (func $test (type 4)
    (local i32 i32 i32 i32 i32 i32 i32 i64 i32 i32 i64 i32 i32 i64 i32 i32 i64 i32 i32 i64 i64 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i64 i32 i32 i64 i32 i32 i64 i32 i32 i64 i32 i32 i64 i64 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)
    global.get $__stack_pointer
    local.set 0
    i32.const 128
    local.set 1
    local.get 0
    local.get 1
    i32.sub
    local.set 2
    local.get 2
    global.set $__stack_pointer
    i32.const 0
    local.set 3
    local.get 3
    i32.load16_u offset=1264
    local.set 4
    i32.const 112
    local.set 5
    local.get 2
    local.get 5
    i32.add
    local.set 6
    local.get 6
    local.get 4
    i32.store16
    local.get 3
    i64.load offset=1256
    local.set 7
    i32.const 104
    local.set 8
    local.get 2
    local.get 8
    i32.add
    local.set 9
    local.get 9
    local.get 7
    i64.store
    local.get 3
    i64.load offset=1248
    local.set 10
    i32.const 96
    local.set 11
    local.get 2
    local.get 11
    i32.add
    local.set 12
    local.get 12
    local.get 10
    i64.store
    local.get 3
    i64.load offset=1240
    local.set 13
    i32.const 88
    local.set 14
    local.get 2
    local.get 14
    i32.add
    local.set 15
    local.get 15
    local.get 13
    i64.store
    local.get 3
    i64.load offset=1232
    local.set 16
    i32.const 80
    local.set 17
    local.get 2
    local.get 17
    i32.add
    local.set 18
    local.get 18
    local.get 16
    i64.store
    local.get 3
    i64.load offset=1224
    local.set 19
    local.get 2
    local.get 19
    i64.store offset=72
    local.get 3
    i64.load offset=1216
    local.set 20
    local.get 2
    local.get 20
    i64.store offset=64
    i32.const 64
    local.set 21
    local.get 2
    local.get 21
    i32.add
    local.set 22
    local.get 22
    local.set 23
    local.get 23
    call $evaluate
    local.set 24
    i32.const 24
    local.set 25
    local.get 24
    local.get 25
    i32.shl
    local.set 26
    local.get 26
    local.get 25
    i32.shr_s
    local.set 27
    i32.const 4
    local.set 28
    local.get 27
    local.get 28
    i32.eq
    local.set 29
    i32.const 1
    local.set 30
    local.get 29
    local.get 30
    i32.and
    local.set 31
    block  ;; label = @1
      local.get 31
      br_if 0 (;@1;)
      i32.const 1107
      local.set 32
      i32.const 1046
      local.set 33
      i32.const 111
      local.set 34
      i32.const 1024
      local.set 35
      local.get 32
      local.get 33
      local.get 34
      local.get 35
      call $__assert_fail
      unreachable
    end
    i32.const 0
    local.set 36
    local.get 36
    i32.load16_u offset=1328
    local.set 37
    i32.const 48
    local.set 38
    local.get 2
    local.get 38
    i32.add
    local.set 39
    local.get 39
    local.get 37
    i32.store16
    local.get 36
    i64.load offset=1320
    local.set 40
    i32.const 40
    local.set 41
    local.get 2
    local.get 41
    i32.add
    local.set 42
    local.get 42
    local.get 40
    i64.store
    local.get 36
    i64.load offset=1312
    local.set 43
    i32.const 32
    local.set 44
    local.get 2
    local.get 44
    i32.add
    local.set 45
    local.get 45
    local.get 43
    i64.store
    local.get 36
    i64.load offset=1304
    local.set 46
    i32.const 24
    local.set 47
    local.get 2
    local.get 47
    i32.add
    local.set 48
    local.get 48
    local.get 46
    i64.store
    local.get 36
    i64.load offset=1296
    local.set 49
    i32.const 16
    local.set 50
    local.get 2
    local.get 50
    i32.add
    local.set 51
    local.get 51
    local.get 49
    i64.store
    local.get 36
    i64.load offset=1288
    local.set 52
    local.get 2
    local.get 52
    i64.store offset=8
    local.get 36
    i64.load offset=1280
    local.set 53
    local.get 2
    local.get 53
    i64.store
    local.get 2
    local.set 54
    local.get 54
    call $evaluate
    local.set 55
    i32.const 24
    local.set 56
    local.get 55
    local.get 56
    i32.shl
    local.set 57
    local.get 57
    local.get 56
    i32.shr_s
    local.set 58
    i32.const 18
    local.set 59
    local.get 58
    local.get 59
    i32.eq
    local.set 60
    i32.const 1
    local.set 61
    local.get 60
    local.get 61
    i32.and
    local.set 62
    block  ;; label = @1
      local.get 62
      br_if 0 (;@1;)
      i32.const 1085
      local.set 63
      i32.const 1046
      local.set 64
      i32.const 116
      local.set 65
      i32.const 1024
      local.set 66
      local.get 63
      local.get 64
      local.get 65
      local.get 66
      call $__assert_fail
      unreachable
    end
    i32.const 1165
    local.set 67
    i32.const 0
    local.set 68
    local.get 67
    local.get 68
    call $printf
    drop
    i32.const 128
    local.set 69
    local.get 2
    local.get 69
    i32.add
    local.set 70
    local.get 70
    global.set $__stack_pointer
    return)
  (func $main (type 0) (param i32 i32) (result i32)
    (local i32)
    call $__original_main
    local.set 2
    local.get 2
    return)
  (table (;0;) 1 1 funcref)
  (memory (;0;) 2)
  (global $__stack_pointer (mut i32) (i32.const 66896))
  (global (;1;) i32 (i32.const 1332))
  (global (;2;) i32 (i32.const 1024))
  (global (;3;) i32 (i32.const 1356))
  (global (;4;) i32 (i32.const 1360))
  (global (;5;) i32 (i32.const 66896))
  (global (;6;) i32 (i32.const 1024))
  (global (;7;) i32 (i32.const 66896))
  (global (;8;) i32 (i32.const 131072))
  (global (;9;) i32 (i32.const 0))
  (global (;10;) i32 (i32.const 1))
  (export "memory" (memory 0))
  (export "__wasm_call_ctors" (func $__wasm_call_ctors))
  (export "push" (func $push))
  (export "st" (global 1))
  (export "pop" (func $pop))
  (export "evaluate" (func $evaluate))
  (export "__original_main" (func $__original_main))
  (export "main" (func $main))
  (export "__main_void" (func $__original_main))
  (export "__indirect_function_table" (table 0))
  (export "__dso_handle" (global 2))
  (export "__data_end" (global 3))
  (export "__stack_low" (global 4))
  (export "__stack_high" (global 5))
  (export "__global_base" (global 6))
  (export "__heap_base" (global 7))
  (export "__heap_end" (global 8))
  (export "__memory_base" (global 9))
  (export "__table_base" (global 10))
  (data $.rodata (i32.const 1024) "test\00Invalid operator\00../files/pop/misc/postfix_evaluation.c\00evaluate(temp2) == 18\00evaluate(temp1) == 4\00Stack overflow...\00Stack underflow...\00All tests have successfully passed!\0a\00\00\00\00\00\00\00\00\00\00\00\00\00\00\002 10 + 9 6 - /\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\004 2 + 3 5 1 - * +\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00"))
